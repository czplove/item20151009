...
extern void SampleApp_SPI_Send_SELF_Command( uint8 *buf, uint8 len );
extern void SampleApp_SPI_SendCommand( uint8 *buf, uint8 len );
/***************************************************************************************************
 *                                          LOCAL FUNCTIONS
 ***************************************************************************************************/
...
void MT_UartAppFlowControl ( bool status )
{
 ...
 #if defined (COORDINATOR) &&(defined (ZTOOL_P2) || defined (ZTOOL_P1)) 
...
//  uint8 word_buffer[8];
  ...
  //databuf[rxlen+1]='\n';                    //一字节存放换行符
  HalUARTRead ( MT_UART_DEFAULT_PORT, databuf+3, rxlen); //读接收缓冲区数据到内存databuf+3
  
    #if defined SECURITY_ENABLE
  #include "TEA.h"//添加TEA 加密 2012 9 26
    if(rxlen>10)
    btea_decrpyt( databuf+3, TEA_key ,rxlen-2);  //解密串口来的数据 最后两位不处理 0D 0A test 2012 9 26
    #endif
 if((databuf[3]==47)&&(databuf[4]==67))// /C  
    SampleApp_SPI_SendCommand( databuf, rxlen+1+2 ); 
   //osal_set_event(SampleApp_TaskID,UART_RX_CMD_CB_EVT);

  else if((databuf[3]==47)&&(databuf[4]==68)) // /D
  SampleApp_SPI_Send_SELF_Command( databuf, rxlen+1+2 );  
  //osal_set_event(SampleApp_TaskID,UART_RX_SELF_CMD_CB_EVT);//TEST THE ASSOIT
 
else osal_mem_free(databuf);
  
}

#endif

#if defined (USB_COORDINATOR)
#include "TEA.h"//添加TEA 加密 2012 9 26
static void rxCB( uint8 port, uint8 event )
{
  ...
  short_ddr_H=(uint8)((short_ddr&0xff00)>>8);
  short_ddr_L=(uint8)short_ddr;
  rxlen=Hal_UART_RxBufLen(0);  //接收缓冲区数据长度,字节为单位
  databuf=osal_mem_alloc(rxlen+1+2);   //多分配3字节,分配如下
  databuf[0]=rxlen;                           //一字节存放数据长度
  databuf[1]=short_ddr_H;              //一字节存放源地址高8位
  databuf[2]=short_ddr_L;              //一字节存放源地址低8位
  HalUARTRead ( 0, databuf+3, rxlen); //读接收缓冲区数据到内存databuf+3
  if(rxlen>10)
  btea_decrpyt( databuf+3, TEA_key ,rxlen-2);  //解密串口来的数据 最后两位不处理 0D 0A test 2012 9 26

#if defined(APP_UART_BUFFER)
   osal_set_event(SampleApp_TaskID,APP_UART_BUFFER_EVT);
#else
