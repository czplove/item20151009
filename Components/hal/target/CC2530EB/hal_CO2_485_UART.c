...
void hal_UARTRead ( uint8 *datBuf)
{
	...
	while(1)
	{
		...
		datBuf[Cnt] = U1DBUF;
		if(0xAA == datBuf[Cnt])// && (0x0D == UARTDatBuf[datCnt - 1]))
		{
			if(0x7E == datBuf[Cnt - 1])
			{
				//receviced
...
HAL_ISR_FUNCTION( halUSART0RxIsr, URX0_VECTOR )
{
	...
	if(0xAA == CO2_UART0_DatBuf_Tem[CO2_UARTRx_datCnt])// && (0x0D == UARTDatBuf[datCnt - 1]))
	{
		if(0x7E == CO2_UART0_DatBuf_Tem[CO2_UARTRx_datCnt - 1])
		{
			//receviced
