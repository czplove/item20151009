...
/* edge interrupt */
#define HAL_KEY_SW_6_EDGEBIT  BV(2)//p1.0~p1.3的触发模式设置 在PICTL的bit2
...
/* edge interrupt */
#define HAL_KEY_SW_8_EDGEBIT  BV(1)//p1.0~p1.3的触发模式设置 在PICTL的bit1
...
#define HAL_KEY_SW_5_ICTLBIT  BV(3) /* P0IEN - P0.1 enable/disable bit BV(1)*/
...
void HalKeyConfig (bool interruptEnable, halKeyCBack_t cback)
{
  ...
  if (Hal_KeyIntEnable)
  {
     ...
     */
    HAL_KEY_SW_5_ICTL |= HAL_KEY_SW_5_ICTLBIT;//P1IEN | 00001000  即P1.3使能中断
    HAL_KEY_SW_5_IEN |= HAL_KEY_SW_5_IENBIT;//使能整个CPU的P1口中断
...
void HalKeyPoll (void)
{
 ...
 //  #endif
  ...
  else
  {
   ...
   if((key_ISR_FLAG & HAL_KEY_SW_5) && (!HAL_PUSH_BUTTON5()))
   {
	   keys |= HAL_KEY_SW_5;
	   key_ISR_FLAG &= ~HAL_KEY_SW_5;
   }
   if (key_ISR_FLAG & HAL_KEY_SW_6)
   {
   		if(PICTL &= 0x04)
		{
			if(P1_4 == 0)
...
//    P1DIR &= ~0X20;//INput
  ...
  if(HAL_KEY_SW_6_PXIFG & HAL_KEY_SW_6_BIT)
  {
	  ...
	  HAL_KEY_SW_6_PXIFG &= ~(HAL_KEY_SW_6_BIT);//清除p1.2标志
	  key_ISR_FLAG |= HAL_KEY_SW_6;
  }
  if(HAL_KEY_SW_5_PXIFG & HAL_KEY_SW_5_BIT)
  {
	  ...
	  //halProcessKeyInterrupt();
	  osal_start_timerEx (Hal_TaskID, HAL_KEY_EVENT, HAL_KEY_DEBOUNCE_VALUE);//25ms以后进入
	  HAL_KEY_SW_5_PXIFG &= ~(HAL_KEY_SW_5_BIT);//清除p1.2标志
	  key_ISR_FLAG |= HAL_KEY_SW_5;
  }
