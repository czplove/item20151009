...
#define HAL_KEY_HOLD_VALUE  9400    //9.4s  按键长按10s（9.4s+600）以后判断为长按按钮操作
 ...
 //  #endif
  ...
  else
  {
   if ((key_ISR_FLAG==HAL_KEY_SW_7) && (!HAL_PUSH_BUTTON6()))//
   {
    ...
    SET_TIME=MAX_SET_TIME;//目的亮度不亮
    //Start_T3();//开启定时器3
    if(DY_TIME !=SET_TIME)//如果和当前的值有变化
    {
     TIMER34_START(3,1);
    }
    //key_set_flag=1;
   ...
   //  keys |= HAL_KEY_SW_8;
    ...
    //TG_Time=4;
    SET_TIME=MIN_SET_TIME;//MIN_SET_TIME;
