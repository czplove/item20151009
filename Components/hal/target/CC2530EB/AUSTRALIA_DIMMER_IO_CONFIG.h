#ifndef AUSTRALIA_DIMMER_IO_CONFIG_H
#define AUSTRALIA_DIMMER_IO_CONFIG_H

//添加的输出
#define LED1_BV           BV(2)//BV(0)//BV(2)//BV(0)//BV(2)//BV(0)//
#define LED1_SBIT        P2_2//P1_0// P2_2//P1_0//P2_2//P1_0//
#define LED1_DDR         P2DIR// P1DIR//P2DIR//P1DIR//P2DIR//P1DIR//
#define LED1_POLARITY     ACTIVE_LOW	//20110721 ACTIVE_HIGH

#define LED2_BV          BV(1)//BV(0)//BV(1)// BV(0)//BV(1)//BV(0)//
#define LED2_SBIT        P2_1//  P1_0// P2_1//P1_0//P2_1//P1_0//
#define LED2_DDR        P2DIR// P1DIR// P2DIR//P1DIR//P2DIR//P1DIR// P2DIR//P0DIR//P2DIR
#define LED2_POLARITY     ACTIVE_LOW


#define LED3_BV           BV(0)
#define LED3_SBIT         P1_0
#define LED3_DDR          P1DIR
#define LED3_POLARITY     ACTIVE_HIGH

#define LED4_BV           BV(0)
#define LED4_SBIT         P1_0
#define LED4_DDR          P1DIR
#define LED4_POLARITY     ACTIVE_HIGH

#define LED5_BV           BV(0)
#define LED5_SBIT         P1_0
#define LED5_DDR          P1DIR
#define LED5_POLARITY     ACTIVE_LOW



#define TG_OUT_BV           BV(1) //PUSLE OUT
#define TG_OUT_SBIT         P0_1
#define TG_OUT_DDR          P0DIR
#define TG_OUT_POLARITY     ACTIVE_HIGH



#define ZERO_CS_BV          BV(0)
#define ZERO_CS_SBIT        P0_0
#define ZERO_CS_POLARITY    ACTIVE_HIGH


#define PUSH2_BV          BV(5)
#define PUSH2_SBIT        P1_5
#define PUSH2_POLARITY    ACTIVE_HIGH



#define PUSH1_BV          BV(6)
#define PUSH1_SBIT        P1_6
#define PUSH1_POLARITY    ACTIVE_HIGH

//new
#define PUSH3_BV          BV(2)
#define PUSH3_SBIT        P1_2
#define PUSH3_POLARITY    ACTIVE_HIGH



#define PUSH4_BV          BV(7)
#define PUSH4_SBIT        P1_7
#define PUSH4_POLARITY    ACTIVE_HIGH

#define PUSH5_BV          BV(7)
#define PUSH5_SBIT        P1_7
#define PUSH5_POLARITY    ACTIVE_HIGH


//添加的输入 




                                                              

#define HAL_IO_CONFIG_INIT()                                     \
{                                                                \
  LED1_DDR |= LED1_BV;                                           \
  LED2_DDR |= LED2_BV;                                           \
  LED5_DDR |= LED5_BV;                                          \
  HAL_TURN_OFF_LED1();                                           \
  HAL_TURN_OFF_LED2();                                           \
  HAL_TURN_OFF_LED5();                                           \
  TG_OUT_DDR |= TG_OUT_BV;                                           \
  TG_OUT_SBIT = 0;\
}   






#endif