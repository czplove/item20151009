...
#define HAL_UART_ISR_RX_AVAIL() isrCfg.rxUsed

#define HAL_UART_ISR_TX_AVAIL() (HAL_UART_ISR_TX_MAX-isrCfg.txUsed)
...
static void HalUARTInitISR(void)
{
  ...
  isrCfg.pRxHead = &isrCfg.rxBuf[0];
  isrCfg.pRxTail = &isrCfg.rxBuf[0];
  isrCfg.rxUsed  = 0;

  isrCfg.pTxHead = &isrCfg.txBuf[0];
  isrCfg.pTxTail = &isrCfg.txBuf[0];
  isrCfg.txUsed  = 0;
...
uint16 HalUARTReadISR(uint8 *buf, uint16 len)
{
  ...
  while((cnt<len)&&(isrCfg.rxUsed>0))
  {
    isrCfg.rxUsed--;    
    cnt++;
    *buf++=*isrCfg.pRxHead;
    if(++isrCfg.pRxHead > &isrCfg.rxBuf[HAL_UART_ISR_RX_MAX-1])
      isrCfg.pRxHead = &isrCfg.rxBuf[0];
  }
...
uint16 HalUARTWriteISR(uint8 *buf, uint16 len)
{
  ...
  for(cnt=0;cnt<len;cnt++)
  {
    ...
    *isrCfg.pTxTail = *buf++;
    isrCfg.txMT = 0;
    if(++isrCfg.pTxTail > &isrCfg.txBuf[HAL_UART_ISR_RX_MAX-1])
      isrCfg.pTxTail = &isrCfg.txBuf[0];
    HAL_EXIT_CRITICAL_SECTION( intState );   // Re-enable interrupts.
...
#endif
{
  ...
  if(isrCfg.rxUsed < HAL_UART_ISR_RX_MAX)
  {
    HAL_ENTER_CRITICAL_SECTION( intState );  // Hold off interrupts.
    isrCfg.rxUsed ++;
    
    *isrCfg.pRxTail = tmp;
    if(++isrCfg.pRxTail > &isrCfg.rxBuf[HAL_UART_ISR_RX_MAX-1])
      isrCfg.pRxTail = &isrCfg.rxBuf[0];    
    HAL_EXIT_CRITICAL_SECTION( intState );   // Re-enable interrupts.  
  }
...
#endif
{
  ...
  else
  {
    ...
    isrCfg.txUsed--;    
    UTXxIF = 0;
    UxDBUF = *isrCfg.pTxHead;
    if(++isrCfg.pTxHead > &isrCfg.txBuf[HAL_UART_ISR_RX_MAX-1])
      isrCfg.pTxHead = &isrCfg.txBuf[0];
    HAL_EXIT_CRITICAL_SECTION( intState );   // Re-enable interrupts. 
  }
