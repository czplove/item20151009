...
#define HAL_KEY_SW_9_ICTLBIT  BV(4) /* P1IEN - P1.4 enable/disable bit */ 
...
uint8 key_double_P1_3_flag = 0;//按键双击状态
  ...
  else
  {
   ...
   if ((key_ISR_FLAG==HAL_KEY_SW_6) && (!HAL_PUSH_BUTTON4()))//初步理解，设置p0.4为下降沿触发，触发唤醒系统后，25ms到这，判断是否为0，是0则读取见键值
   {
    //keys |= HAL_KEY_SW_6;
     ...
     key_p1_3_ISR_FLAG++;//test 测试单击 双击等程序
...
void HalKeyMultiClickCheck(void)
{
 ...
 {  
  ...
  {  
      keys |= HAL_KEY_SW_6;
      key_double_P1_3_flag=key_p1_3_ISR_FLAG;
