...
#define HAL_KEY_SW_9_ICTLBIT  BV(3) /* P1IEN - P1.2 enable/disable bit */ 
...
/* edge interrupt */
#define HAL_KEY_SW_8_EDGEBIT  BV(1)//p1端口的触发模式设置 在PICTL的低第三位（位2）P1.5为高4位地址
...
#define HAL_KEY_SW_8_ICTLBIT  BV(2) /* P1IEN - P1.4 enable/disable bit */ 
...
/* edge interrupt */
#define HAL_KEY_SW_7_EDGEBIT  BV(3)//p2端口的触发模式设置 在PICTL的低第三位
...
#define HAL_KEY_SW_7_ICTLBIT  BV(0) /* P1IEN - P1.4 enable/disable bit */ 
...
uint8 KEY_NUMBER_FLAG_A = 0;//按键标志
...
#endif		
		HAL_KEY_SW_7_ICTL |= HAL_KEY_SW_7_ICTLBIT;//P1IEN | 00100000  即P1.5使能中断
...
void HalKeyPoll (void)
{
	...
	else
	{
		...
		{
			KEY_NUMBER_FLAG_A = HAL_KEY_SW_8;
		...
		{
                        KEY_NUMBER_FLAG_A = HAL_KEY_SW_9;
//			keys |= HAL_KEY_SW_9;
//			if(key_holdtime_waiting_flag)
//			{
//				key_holdtime_waiting_flag = 0;//在等待长按时间时再次按下按钮 此时需要清等待长按时间标志以及取消之前设置的等待长按时间满以后进入事件
...
//			}
		...
		else if ((key_ISR_FLAG==HAL_KEY_SW_7) && (!(HAL_PUSH_BUTTON1())))
		{
                        KEY_NUMBER_FLAG_A = HAL_KEY_SW_7;
//			keys |= HAL_KEY_SW_7;
//			if(key_holdtime_waiting_flag)
//			{
//				key_holdtime_waiting_flag = 0;//在等待长按时间时再次按下按钮 此时需要清等待长按时间标志以及取消之前设置的等待长按时间满以后进入事件
